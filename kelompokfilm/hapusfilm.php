...
if(isset($_GET['id_kelompokfilm'])){
	//membuat variabel $id yang menyimpan nilai dari $_GET['id']
	$id_kelompokfilm = $_GET['id_kelompokfilm'];

	//melakukan query ke database, dengan cara SELECT data yang memiliki id yang sama dengan variabel $id
	$cek = mysqli_query($connect, "SELECT * FROM kelompokfilm WHERE id_kelompokfilm='$id_kelompokfilm'") or die(mysqli_error($connect));

	//jika query menghasilkan nilai > 0 maka eksekusi script di bawah
	if(mysqli_num_rows($cek) > 0){
		//query ke database DELETE untuk menghapus data dengan kondisi id=$id
		$del = mysqli_query($connect, "DELETE FROM kelompokfilm WHERE id_kelompokfilm='$id_kelompokfilm'") or die(mysqli_error($connect));
		if($del){
